...
def write_psql(df, db_name: str="test", 
    ...
    """
    logging.info("write_psql function is started.")
    ...
    try:
        ...
        logging.info("PostgreSQL {db_name}.{table_name} is saved.")
    except Exception as e:
        logging.error(f"Dataframe cannot {mode} to {db_name}.{table_name}!\n{e}")
    finally:
        logging.info("write_psql function was finished.")
...
def write_hive(df, format_: str="orc", 
    ...
    """
    logging.info("write_hive function is started.")
    ...
    try:
        ...
        logging.info("write_hive function was finished.")
...
def write_deltalake(df, mode: str="overwrite", 
    ...
    """
    logging.info("write_deltalake function is started.")
    ...
    try:
        ...
        logging.error(f"Dataframe cannot {mode} to {path}!\n{e}")
    finally:
        logging.info("write_deltalake function was finished.")
...
def upsert_deltalake(df, new_df, join_expr: str):
    ...
    """
    logging.info("upsert_deltalake function is started.")
    
    try:
        logging.warn("Old dataframe alias: df\nNew dataframe alias:new_df")
        (
             df.alias("df") \
        ...
        logging.error(f"Dataframe cannot upserted to dataframe!\n{e}")
    finally:
        logging.info("upsert_deltalake function was finished.")
...
def read_psql(spark, db_name: str="test", 
    ...
    Spark.Dataframe
    """
    logging.info("read_psql function is started.")
    jdbc_url = f"jdbc:postgresql://localhost:5432/{db_name}"
    ...
    try:
        ...
        logging.error(f"Dataframe cannot read from p-sql!\n{e}")
    finally:
        logging.info("read_psql function was finished.")
...
def read_hive(spark, db_name: str="test", 
    ...
    Spark.Dataframe
    """
    logging.info("read_hive function is started.")
    
    try:
        return spark.sql(f"select * from {db_name}.{table_name}")
    except Exception as e:
        logging.error(f"Dataframe cannot read from apache hive!\n{e}")
    finally:
        logging.info("read_hive function was finished.")
...
def read_delta(spark, path: str):
    ...
    Spark.Dataframe
    """
    logging.info("read_delta function is started.")
    ...
    try:
        ...
        logging.error(f"Dataframe cannot read from deltalake!\n{e}")
    finally:
        logging.info("read_delta function was finished.")
